...
const GroupDetails = ({ route, navigation }) => {
  ...
  const handleApi=(type)=>{
    setIsLoading(true);
    if(type==="RefreshCode"){
      axios.post(`http://192.168.0.192:8080/api/group/changeGroupCode/1`)
      .then(response=>{
        setgroupData(response.data);
        setIsLoading(false);
      })
  ...
  return (
    <View style={style.GroupDetails}>
      ...
      <View style={style.GroupDetailsTop}>
        ...
        <Text onPress={()=>handleApi("RefreshCode")} style={style.GroupDetailsTopDetails}>Refresh Group join code</Text>
      </View>
      {/* Middle Section */}
          ...
          <ScrollView>
            {
              data.map((val,i)=>(
                ...
                i={i} img={lastSelect==="members"?"":forwardIcon2} key={val.id} id={val.id}
